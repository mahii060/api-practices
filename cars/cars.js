...
const showData = () => {
    ...
    cars.slice(0, 3).forEach((car) => {
        console.log(car._id)
        const { _id, name, price, imageURL, description } = car;
        const carContainer = document.createElement('div');
        carContainer.classList.add('col');
        carContainer.innerHTML = `
        <div class="card">
            <img src="${car.imageURL}" class="card-img-top" alt="...">
            <div class="card-body">
                <h5 class="card-title">Car Name: ${name}</h5>
                <p class="card-text h4">Car Detail: ${description}</p>
                <button class ="btn btn-warning">Car Price: ${price}</button>
            </div>
        </div>
        `;
